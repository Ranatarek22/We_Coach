...
//        $manager->save();
    ...
    public function internsFilter(Request $request){
        ...
        $internsWecoach=Intern::all()->where("academyID","1")->where("branch",$branch);
        $internsWaves=Intern::all()->where("academyID","2")->where("branch",$branch);
        }

        return response()->json(["interns"=>count($internsWecoach)+count($internsWaves),"internsWecoach"=>count($internsWecoach),"internsWaves"=>count($internsWaves)]);
    }
